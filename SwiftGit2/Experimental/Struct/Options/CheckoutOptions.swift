...
extension CheckoutOptions {
    func with_git_checkout_options<T>(_ body: (inout git_checkout_options) -> T) -> T {
        ...
        checkout_options.progress_payload   = self.toRetainedPointer()              // RETAIN
...
fileprivate func checkoutProgressCallback(path: UnsafePointer<Int8>?, completedSteps: Int, totalSteps: Int, payload: UnsafeMutableRawPointer?) {
