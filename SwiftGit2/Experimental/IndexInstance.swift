...
public extension Index {
    ...
    func add(path: String) -> Result<Void, NSError> {
        let dir = path
    ...
    func remove(path: String) -> Result<Void, NSError> {
        let dir = path
        var dirPointer = UnsafeMutablePointer<Int8>(mutating: (dir as NSString).utf8String)
        var paths = git_strarray(strings: &dirPointer, count: 1)
    ...
    func clear() -> Result<Void, NSError> {
        return _result((), pointOfFailure: "git_index_clear") {
            git_index_clear(pointer)
        }
    }

    private func write() -> Result<Void, NSError> {
