...
public final class Repository {
	...
	public class func create(at url: URL) -> Result<Repository, SwiftGit2Error> {
		...
		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source: .git_repository_init))
		}
	...
	///
	/// Returns a `Result` with a `Repository` or an error.
	public class func at(_ url: URL) -> Result<Repository, SwiftGit2Error> {
		var pointer: OpaquePointer? = nil
		let result = url.withUnsafeFileSystemRepresentation {
		...
		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source: .git_repository_open))
		}
	...
	public class func clone(from remoteURL: URL, to localURL: URL, localClone: Bool = false, bare: Bool = false,
	                        ...
	                        checkoutProgress: CheckoutProgressBlock? = nil) -> Result<Repository, SwiftGit2Error> {
		...
		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source: .git_clone))
		}
	...
	private func withGitObject<T>(_ oid: OID, type: git_object_t,
	                              transform: (OpaquePointer) -> Result<T, SwiftGit2Error>) -> Result<T, SwiftGit2Error> {
		var pointer: OpaquePointer? = nil
		var oid = oid.oid
		let result = git_object_lookup(&pointer, self.pointer, &oid, type)

		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source: .git_object_lookup))
		}
	...
	private func withGitObject<T>(_ oid: OID, type: git_object_t, transform: (OpaquePointer) -> T) -> Result<T, SwiftGit2Error> {
		return withGitObject(oid, type: type) { .success(transform($0)) }
	}

	private func withGitObjects<T>(_ oids: [OID], type: git_object_t, transform: ([OpaquePointer]) -> Result<T, SwiftGit2Error>) -> Result<T, SwiftGit2Error> {
	...
	public func object(_ oid: OID) -> Result<ObjectType, SwiftGit2Error> {
		return withGitObject(oid, type: GIT_OBJECT_ANY) { object in
			...
			return .failure(.unknownObjectType(type, oid))
		}
	}
	...
	public func blob(_ oid: OID) -> Result<Blob, SwiftGit2Error> {
	...
	public func commit(_ oid: OID) -> Result<Commit, SwiftGit2Error> {
	...
	public func tag(_ oid: OID) -> Result<Tag, SwiftGit2Error> {
	...
	public func tree(_ oid: OID) -> Result<Tree, SwiftGit2Error> {
	...
	public func object<T>(from pointer: PointerTo<T>) -> Result<T, SwiftGit2Error> {
	...
	///
	/// Returns the object if it exists, or an error.
	public func object(from pointer: Pointer) -> Result<ObjectType, SwiftGit2Error> {
		switch pointer {
		case let .blob(oid):
	...
	public func allRemotes() -> Result<[Remote], SwiftGit2Error> {
		let pointer = UnsafeMutablePointer<git_strarray>.allocate(capacity: 1)
		let result = git_remote_list(pointer, self.pointer)

		guard result == GIT_OK.rawValue else {
			pointer.deallocate()
			return .failure(.init(libgitErrorCode: result, source: .git_remote_list))
		}

		let strarray = pointer.pointee
		let remotes: [Result<Remote, SwiftGit2Error>] = strarray.map {
	...
	private func remoteLookup<A>(named name: String, _ callback: (Result<OpaquePointer, SwiftGit2Error>) -> A) -> A {
		var pointer: OpaquePointer? = nil
		...
		guard result == GIT_OK.rawValue else {
			return callback(.failure(.init(libgitErrorCode: result, source: .git_remote_lookup)))
		}
	...
	public func remote(named name: String) -> Result<Remote, SwiftGit2Error> {
		return remoteLookup(named: name) { $0.map(Remote.init) }
	}

	/// Download new data and update tips
	public func fetch(_ remote: Remote) -> Result<(), SwiftGit2Error> {
		return remoteLookup(named: remote.name) { remote in
			remote.flatMap { pointer in
				...
				guard result == GIT_OK.rawValue else {
					return .failure(.init(libgitErrorCode: result, source: .git_remote_fetch))
				}
				return .success(())
			}
		}
	...
	public func references(withPrefix prefix: String) -> Result<[ReferenceType], SwiftGit2Error> {
		let pointer = UnsafeMutablePointer<git_strarray>.allocate(capacity: 1)
		let result = git_reference_list(pointer, self.pointer)

		guard result == GIT_OK.rawValue else {
			pointer.deallocate()
			return .failure(.init(libgitErrorCode: result, source: .git_reference_list))
		}
	...
	public func reference(named name: String) -> Result<ReferenceType, SwiftGit2Error> {
		var pointer: OpaquePointer? = nil
		let result = git_reference_lookup(&pointer, self.pointer, name)

		guard result == GIT_OK.rawValue else {
			return Result.failure(.init(libgitErrorCode: result, source: .git_reference_lookup))
		}
	...
	public func localBranches() -> Result<[Branch], SwiftGit2Error> {
	...
	public func remoteBranches() -> Result<[Branch], SwiftGit2Error> {
	...
	public func tag(named name: String) -> Result<TagReference, SwiftGit2Error> {
	...
	public func HEAD() -> Result<ReferenceType, SwiftGit2Error> {
		var pointer: OpaquePointer? = nil
		let result = git_repository_head(&pointer, self.pointer)
		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source:.git_repository_head))
		}
		let value = referenceWithLibGit2Reference(pointer!)
		git_reference_free(pointer)
	...
	public func setHEAD(_ oid: OID) -> Result<(), SwiftGit2Error> {
		var oid = oid.oid
		let result = git_repository_set_head_detached(self.pointer, &oid)
		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source: .git_repository_set_head))
		}
		return .success(())
	}
	...
	/// :returns: Returns a result with void or the error that occurred.
	public func setHEAD(_ reference: ReferenceType) -> Result<(), SwiftGit2Error> {
	...
	/// :param: progress A block that's called with the progress of the checkout.
	/// :returns: Returns a result with void or the error that occurred.
	public func checkout(strategy: CheckoutStrategy, progress: CheckoutProgressBlock? = nil) -> Result<(), SwiftGit2Error> {
		var options = checkoutOptions(strategy: strategy, progress: progress)

		let result = git_checkout_head(self.pointer, &options)
		guard result == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: result, source: .git_checkout_head))
		}
	...
	public func checkout(_ oid: OID, strategy: CheckoutStrategy,
	                     progress: CheckoutProgressBlock? = nil) -> Result<(), SwiftGit2Error> {
	...
	func unsafeIndex() -> Result<OpaquePointer, SwiftGit2Error> {
		var index: OpaquePointer? = nil
		let result = git_repository_index(&index, self.pointer)
		guard result == GIT_OK.rawValue && index != nil else {
			return .failure(.init(libgitErrorCode: result, source: .git_repository_index))
		}
		return .success(index!)
	}

	/// Stage the file(s) under the specified path.
	public func add(path: String) -> Result<(), SwiftGit2Error> {
		...
		return unsafeIndex().flatMap { index in
			...
			guard addResult == GIT_OK.rawValue else {
				return .failure(.init(libgitErrorCode: addResult, source:.git_index_add_all))
			}
			// write index to disk
			let writeResult = git_index_write(index)
			guard writeResult == GIT_OK.rawValue else {
				return .failure(.init(libgitErrorCode: writeResult, source: .git_index_write))
			}
			return .success(())
		}
	}
	...
	public func commit(
		...
		return signature.makeUnsafeSignature().flatMap { signature in
			...
			guard lookupResult == GIT_OK.rawValue else {
				return .failure(.init(libgitErrorCode: lookupResult, source: .git_tree_lookup))
			}
			...
			for parentCommit in parents {
				...
				guard lookupResult == GIT_OK.rawValue else {
					return .failure(.init(libgitErrorCode: lookupResult, source: .git_commit_lookup))
				}
			...
			return parentsContiguous.withUnsafeBufferPointer { unsafeBuffer in
				...
				guard result == GIT_OK.rawValue else {
					return .failure(.init(libgitErrorCode: result, source: .git_commit_create))
				}
	...
	public func commit(message: String, signature: Signature) -> Result<Commit, SwiftGit2Error> {
		return unsafeIndex().flatMap { index in
			defer { git_index_free(index) }
			var treeOID = git_oid()
			let treeResult = git_index_write_tree(&treeOID, index)
			guard treeResult == GIT_OK.rawValue else {
				return .failure(.init(libgitErrorCode: treeResult, source: .git_index_write_tree))
			}
			...
			guard nameToIDResult == GIT_OK.rawValue else {
				return .failure(.init(libgitErrorCode: nameToIDResult, source: .git_reference_name_to_id))
			}
	...
	public func diff(for commit: Commit) -> Result<Diff, SwiftGit2Error> {
				...
				case .success(let newDiff):
					if mergeDiff == nil {
						...
						guard mergeResult == GIT_OK.rawValue else {
							return .init(libgitErrorCode: mergeResult, source: .git_diff_merge)
						}
					}
					return nil
				}
	...
	private func diff(from oldCommitOid: OID?, to newCommitOid: OID?, transform: (Result<OpaquePointer, SwiftGit2Error>) -> SwiftGit2Error?) -> SwiftGit2Error? {
		...
		guard diffResult == GIT_OK.rawValue else {
			return transform(.failure(.init(libgitErrorCode: diffResult, source: .git_diff_tree_to_tree)))
		}
	...
	/// Memory safe
	private func diff(from oldCommitOid: OID?, to newCommitOid: OID?) -> Result<Diff, SwiftGit2Error> {
		assert(oldCommitOid != nil || newCommitOid != nil, "It is an error to pass nil for both the oldOid and newOid")
		...
		return .failure(.diffError)
	}

	private func processTreeToTreeDiff(_ diffResult: Int32, diff: OpaquePointer?) -> Result<Diff, SwiftGit2Error> {
		guard diffResult == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: diffResult, source: .git_diff_tree_to_tree))
		}
	...
	private func processDiffDeltas(_ diffResult: OpaquePointer) -> Result<[Diff.Delta], SwiftGit2Error> {
	...
	private func safeTreeForCommitId(_ oid: OID) -> Result<Tree, SwiftGit2Error> {
	...
	private func unsafeTreeForCommitId(_ oid: OID) -> Result<OpaquePointer, SwiftGit2Error> {
		var commit: OpaquePointer? = nil
		var oid = oid.oid
		let commitResult = git_object_lookup(&commit, self.pointer, &oid, GIT_OBJECT_COMMIT)
		guard commitResult == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: commitResult, source: .git_object_lookup))
		}
		...
		guard treeResult == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: treeResult, source: .git_object_lookup))
		}
	...
	public func status(options: StatusOptions = [.includeUntracked]) -> Result<[StatusEntry], SwiftGit2Error> {
		...
		guard optionsResult == GIT_OK.rawValue else {
			return .failure(.init(libgitErrorCode: optionsResult, source: .git_status_init_options))
		}
		...
		guard statusResult == GIT_OK.rawValue, let unwrapStatusResult = unsafeStatus else {
			return .failure(.init(libgitErrorCode: statusResult, source: .git_status_list_new))
		}
	...
	public static func isValid(url: URL) -> Result<Bool, SwiftGit2Error> {
		...
		case GIT_OK.rawValue:
			return .success(true)
		default:
			return .failure(.init(libgitErrorCode: result, source: .git_repository_open_ext))
		}
	}
}

private extension Array {
	func aggregateResult<Value>() -> Result<[Value], SwiftGit2Error> where Element == Result<Value, SwiftGit2Error> {
		var values: [Value] = []
		for result in self {
			switch result {
			case .success(let value):
